...
class BaseSet(Dataset):
    def __init__(self, mode="train", cfg=None, transform=None):
        ...
        self.size = cfg.INPUT_SIZE #self.input_size
        ...
        if self.mode == "train":
            ...
            self.json_path = cfg.TRAIN_JSON
                                
        elif "valid" in self.mode:
            print("Loading valid data ...", end=" ")
            self.json_path = cfg.VALID_JSON
            size = self.size
        ...
        if  mode == "train":
            ...
            num_list, cat_list = get_category_list(self.get_annotations(), self.num_classes)
    ...
    def update(self, epoch):
        self.epoch = epoch
        #if self.cfg.sampler_type == "progressive":
        #    self.progress_p = epoch/self.cfg.epochs * self.class_p + (1-epoch/self.cfg.epochs)*self.instance_p
    ...
    def update_transform(self, input_size=None):
        ...
        transform_ops = (
            self.cfg.TRAIN_TRANSFORMS
            if self.mode == "train"
            else self.cfg.TEST_TRANSFORMS
        )
    ...
    def imread_with_retry(self, fpath):
        ...
        for k in range(retry_time):
            try:
                img = Image.open(fpath) #cv2.imread(fpath)
                if img is None: 
